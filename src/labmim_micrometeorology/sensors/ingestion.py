...
def merge_dat_files(
    ...
    **kwargs:
    ...
    # Reset index for merge_ordered, then restore
    dfs_reset = [df.reset_index().rename(columns={"index": "TIMESTAMP"}) for df in dfs]

    merged = reduce(lambda left, right: pd.merge_ordered(left, right), dfs_reset)
