...
class SequenceDatasetMeta:
    ...
    def load(cls, path: str | Path) -> SequenceDatasetMeta:
        ...
        p = Path(path)
        if not (p / "sequences.npz").exists() and (p / "windowed_sequences.npz").exists():
            return cls._load_windowed_compat(p)
    ...
    @classmethod
    def _load_windowed_compat(cls, path: Path) -> SequenceDatasetMeta:
        """Load new lazy artifacts through the legacy dense metadata API."""
        from solrad_correction.features.sequence import create_sequences

        meta = WindowedSequenceDatasetMeta.load(path)
        if meta.target_offset not in (None, meta.sequence_length):
            raise ValueError("Legacy dense load supports only target_offset == sequence_length")
        features, targets = create_sequences(meta.features, meta.targets, meta.sequence_length)
        return cls(
            X_raw=features,
            y_raw=targets,
