...
def diurnal_cycle(
    ...
    return df[cols].groupby(df.index.hour).mean()  # type: ignore
...
def monthly_means(
    ...
    cols = columns or list(df.columns)
    cols = [c for c in cols if c in df.columns]
    return df[cols].groupby(df.index.month).mean()  # type: ignore
...
def seasonal_groups(
    ...
    return {name: df[df.index.month.isin(months)] for name, months in seasons.items()}  # type: ignore
