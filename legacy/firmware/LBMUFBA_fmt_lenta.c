...
int main (void){
	...
	char * nome = "../LBM_lenta_2019.dat";
	 ...
	 if (( pb = fopen ("../formatado/LBMUFBA_lenta_2019.dat", "w" )) == NULL ) { /* Abre o arquivo ../formatado/LBM_lenta*.dat para salvar o arquivo compilado */
		printf ("\nErro ao abrir o arquivo - escrita.\n" );
		return 1;
	}
	 while (n<944) { /* Exclui as primeiras linhas texto do arquivo gerado pelo datalogger */
         c = fgetc (pa);
         n++;
         }
         while (!feof (pa)) {
	 ...
	 fclose (pb); /* Fecha o arquivo LBM_lenta.dat */
	 fclose (pa); /* Fecha o arquivo LBM_lenta.dat */
 return 0;
}
